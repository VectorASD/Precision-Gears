...
def _get_event_loop():
    """Safely get the current event loop, creating one if necessary.

    This helper function handles Python 3.10+ where get_event_loop()
    ...
    # Try to get the current event loop via policy
    try:
        ...
        # Python 3.12+ raises RuntimeError if no loop exists
        pass

    # Last resort: create and set a new event loop
    try:
        ...
        log.error(f"Failed to create event loop: {e}")
        raise
...
def setup_asyncio_executor():
    ...
    # First ensure an event loop exists
    _get_event_loop()

    if sys.platform == "win32":
        # On Windows, close the default loop and create ProactorEventLoop
        # ProactorEventLoop is required for subprocess support on Windows
        # Source: https://docs.python.org/3/library/asyncio-subprocess.html
        try:
            policy = asyncio.get_event_loop_policy()
            existing_loop = policy.get_event_loop()
        ...
        # Get the event loop we just ensured exists
...
class AsyncModalOperatorMixin:
    ...
    def _new_async_task(
        ...
        # Ensure event loop exists before creating Future and scheduling task
        _get_event_loop()

        # Download the previews asynchronously.
        # Note: loop parameter removed in Python 3.10+
